...
contract EndpointV2ONFT721Mock is ILayerZeroEndpointV2, MessagingContext {
    using ExecutorOptions for bytes;
    using ONFTComposeMsgCodec for bytes;
    using ONFTComposeMsgCodec for bytes32;
    using ONFTComposeMsgCodec for address;
    ...
    function send(
        ...
        console.log("send _params.options.length", _params.options.length);
        ...
        console.log("receipt.fee.nativeFee", receipt.fee.nativeFee);
        ...
        EndpointV2ONFT721Mock(lzEndpoint).receivePayload{ value: dstAmount }(
            origin,
    ...
    function _quote(
        ...
        console.log("run _quote");
        (bytes memory executorOptions, ) = splitOptions(_params.options);
        console.log("executorOptions.length", executorOptions.length);
        // 2) get Executor fee
    ...
    function splitOptions(bytes calldata _options) internal view returns (bytes memory, WorkerOptions[] memory) {
        console.log("run splitOptions");
        console.log("_options.length", _options.length);
        (bytes memory executorOpts, bytes memory dvnOpts) = decode(_options);
        console.log("executorOpts.length", executorOpts.length);
        console.log("dvnOpts.length", dvnOpts.length);
    ...
    function decode(
        bytes calldata _options
    ) internal view returns (bytes memory executorOptions, bytes memory dvnOptions) {
        ...
        console.log("decode _options.length", _options.length);
        ...
        if (optionsType == UlnOptions.TYPE_3) {
            console.log("optionsType == UlnOptions.TYPE_3");
            unchecked {
    ...
    function decodeLegacyOptions(
        uint16 _optionType,
        bytes calldata _options
    ) internal view returns (bytes memory executorOptions) {
        console.log("_optionType", _optionType);
    ...
    function executeNativeAirDropAndReturnLzGas(
        ...
        console.log("run executeNativeAirDropAndReturnLzGas");
    ...
    function _executeNativeAirDropAndReturnLzGas(
        ...
        while (cursor < _options.length) {
            (uint8 optionType, bytes calldata option, uint256 newCursor) = _options.nextExecutorOption(cursor);
            console.log("_executeNativeAirDropAndReturnLzGas optionType", optionType);
            cursor = newCursor;
            ...
            if (optionType == ExecutorOptions.OPTION_TYPE_LZRECEIVE) {
                ...
                console.log("revert Executor_UnsupportedOptionType");
                revert IExecutorFeeLib.Executor_UnsupportedOptionType(optionType);
            }
        }
