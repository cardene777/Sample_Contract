...
describe('MyONFT721 Test', function () {
    ...
    it('should send a token from A address to B address via each ONFT721', async function () {
        ...
        const [nativeFee] = await holeskyONFT.quoteSend(sendParam, false)

        console.log(`nativeFee: ${nativeFee}`)

        const tx = await holeskyONFT
            .connect(walletOwnerA)
            .send(sendParam, [nativeFee, 0], walletOwnerA.address, { value: nativeFee })
